# Teste de aptidões

## Introdução
Para avaliar um pouco do seu conhecimento técnico e a sua forma de resolver problemas, propomos um breve desafio.

O desafio consite em criar um banner em 3 formatos diferentes e posteriormente transformá-lo em um vídeo de 15 segundos.

Com isso, o nosso intuito é analisar as suas habilidades para criar e desdobrar uma mesma peça em diferentes formatos.


##  Vamos ao desafio:

O cliente John&Cia solicitou banners para divulgação de um dos seus principais produtos: **As Trufas Importadas Hazel.**

### BRIEFING

#### Formatos solicitados:

- 1500x630px (banner para site desktop)
- 500x300px (banner para site mobile)
- 1200x1200 (post feed)
- 1080x1920px (vídeo de 15 segundos para story)


#### Conteúdo:

Logo do Cliente [baixe aqui](john-e-cia/logo-john-e-cia.png)
Texto: As incríveis e incomparáveis trufas Hazel, você só encontra aqui.
Preço: R$54,90
CTA: Aproveite

Imagens do produto:

* [Imagem 1](produto/imagem-trufa-1.jpg)
* [Imagem 2](produto/imagem-trufa-2.jpg))
* [Imagem 3](produto/imagem-trufa-3.jpg))
* [Imagem 4](produto/imagem-trufa-4.jpg))
* [Imagem 5](produto/imagem-trufa-5.jpg))

O produto e as imagens acima são fictícios, portanto, caso prefira, você pode utilizar outras imagens para simular o produto (desde que seja trufa de chocolate).
O conteúdo dos banners é o mesmo, você só precisará adaptar para os formatos solicitados.


##  ATENÇÃO:

Os banners devem ser criados no Photoshop e o vídeo no Premiere.

Você deve entregar os arquivos abertos e também exportar as imagens para PNG ou JPG e o vídeo para MP4.


## Dúvidas?
É só mandar um e-mail para <jobs@addsales.com>.

Obrigado e boa sorte!
